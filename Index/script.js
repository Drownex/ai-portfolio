...
const GEMINI_KEY = "AIzaSyA29gW-f-_q8hINsSe2jE3CJgoEH4FGqls";
// Nama model Gemini yang digunakan
const GEMINI_MODEL = "gemini-2.5-flash";
// Konstanta pelembutan perubahan pada persentase kelas
const SMOOTHING = 0.2;
...
let lastPredict = false; // Check apabila prediksi terakhir telah dijalankan
let smoothedPredictions; // Array dengan isi persentase setiap kelas saat ini
...
async function initTeachableMachine() {
    ...
    // Mematikan Demo AI apabila tidak digunakan
    if(isRunning){
        isRunning = false;
    ...
    try {
        ...
        smoothedPredictions = Array(maxPredictions).fill(0);
...
async function predictionLoop() {
    if (!isRunning) return;
    if(webcam != undefined){ // ambil frame terbaru dari kamera
        webcam.update();
    }
    ...
    for (let i = 0; i < maxPredictions; i++) {
        ...
        // Update prediksi halus untuk progress bar-nya
        smoothedPredictions[i] = (smoothedPredictions[i] * (1-SMOOTHING)) + (pct * SMOOTHING);
        ...
        <div style='background:#e2e8f0;border-radius:4px;height:6px;'>
            <div style='background:#667eea;width:${smoothedPredictions[i]}%;height:100%;border-radius:4px;',transition:width 0.1s'>
            </div>
        </div>
        `;
    }
